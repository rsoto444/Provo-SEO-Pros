...
- **GoHighLevel chat widget** (id 6ab5849136d62147628e29db) is on every page, as required by GoHighLevel's A2P campaign setup. It collects SMS consent inside the chat.
- **A2P opt-in decision (Thursday 24 September):** the GoHighLevel chat widget is the ONLY SMS opt-in. Website forms no longer ask for phone or show SMS boxes (hidden in code, not deleted). Widget is on every page except /thank-you/ (which has the booking calendar).
- **Address now shown on the site** (needed for A2P brand check): 2650 W 820 N, Provo, UT 84601 in the footers, contact page, site settings and llms.txt. Brand registered as sole proprietor with this address and +1 866-402-6849.
...
- **Keyword research (Saturday 26 September 2026):** local first (Provo and Utah County map), national terms mainly for blog posts. Service list for the map confirmed: SEO & AI Search Optimization; Google Business Profile Management; Website Design & Conversion Optimization; CRM Automation & Lead Follow-Up; PPC Advertising Management; Social Media Management; Growth Engine System (Local Visibility Blueprint, Guided Implementation, Local Authority Managed Growth); Local Visibility Starter; Free Written SEO Audit; Organic Lead Generation System. Nothing excluded. Data: free route (Search Console + Google Keyword Planner, owner has a Google Ads account); Semrush has no API units, Ahrefs plan has no API access.
- **Keyword data source (Saturday 26 September 2026):** DataForSEO, login in .env (never committed). Market: US primary with Canada checked in the same run (owner chose B); Utah County (location 9059681) for local numbers. First map run used about $0.99 of the $1 trial credit, balance about $0.01; top up before the next run. Authority score not measured (needs DataForSEO backlinks access), so blog posts use the flat difficulty ceiling of 30.
